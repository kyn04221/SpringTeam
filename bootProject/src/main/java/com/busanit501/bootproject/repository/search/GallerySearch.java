package com.busanit501.bootproject.repository.search;
...
public interface GallerySearch {
    ...
    Page<Gallery> search(Pageable pageable);

    //String[] types , "t", "c", "tc"
    // Pageable -> 페이징 하기 위한 재료. 현재 페이지, 페이지 보여줄 갯수, 정렬
    // Page -> 1) 페이징된 결과물 10개 2) 전체 갯수 3) 현제 페이지, 등. 정보 조회 가능.
    Page<Gallery> searchAll(String[] types, String keyword, Pageable pageable);
    ...
    // Page<GalleryListReplyCountDTO> searchWithReplyCount(String[] types, String keyword, Pageable pageable);

    // 게시글 + 댓글 갯수 + 첨부 이미지
    Page<GalleryListAllDTO> searchWithAll(String[] types, String keyword, Pageable pageable);
